...
export async function POST(request: Request) {
  ...
  try {
    ...
    const litellmBody = {
      key: keyData.key, // required field
      ...
      temp_budget_increase: keyData.temp_budget_increase ? Number(keyData.temp_budget_increase) : undefined,
      temp_budget_expiry: keyData.temp_budget_expiry
    };
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update API key: ${responseText}`);
    }
    ...
    console.error('\nError in API key update route:', error);
